...
@Observable
class HomeViewModel {
    private let logger = Logger(subsystem: "com.mebeatme.ios", category: "HomeViewModel")
    ...
    var performanceInsights: [String] {
        ...
        // PPI insight
        if let ppi = highestPPILast90Days {
            insights.append("Your highest PPI of \(Units.formatPPI(ppi)) puts you in the \(performanceLevel.rawValue) category")
        }
        
        // Distance insight
        if totalDistanceKm > 0 {
            insights.append("You've run a total of \(String(format: "%.1f", totalDistanceKm))km")
        }
        
        // Consistency insight
        if runsLast7Days.count > 0 {
            insights.append("You've run \(runsLast7Days.count) times in the last 7 days")
        }
